...
def get_token():
    ...
    if 'username' and 'userlevel' and 'room' in params:
        connection_metadata = "username={username},userLevel={userlevel}".format(username=params["username"], userlevel=params["userlevel"])
        ...
        if params['room'] not in rooms:
            ...
            token = opentok.generate_token(session.session_id, Roles.publisher, None, connection_metadata)
            ...
            token = opentok.generate_token(rooms[params['room']], Roles.publisher, None, connection_metadata)
        ...
        if params["room"] in rooms:
            if params["playlist_id"] in playlist_videos:
                ...
                try:
                    ...
                    data = json.loads(requests.get('https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId={}&key={}'.format(params["playlist_id"],environ.get("YOUTUBE_API_KEY"))).text)
                    playlist_videos[params["playlist_id"]] = data
                    #create a litte wrapper
                    data = {"status":"success", "playlist":data}
                except:
                    print("Error when trying to return videos")
                    data = { "status":"error", "message": "Error when trying to return videos" }
...
def on_create(data):
    ...
    if data["room"] in rooms:
        ...
        emit('join.room',{'message': 'Conneted to room: {room} / {room_id}'.format(room=data["room"], room_id=rooms[data["room"]]), 'email':data["email"], 'queue':queue[rooms[data["room"]]] })
    else:
...
def on_add_queue(data):
    ...
    queue[rooms[data["room"]]].append({'vid':data["vid"], 'vname':data["vname"], 'vimage':data["vimage"], 'username':data["username"], 'useremail':data["useremail"], 'videoselector':data["videoselector"]})
    emit('add.to.queue', {'vid':data["vid"], 'vname':data["vname"], 'vimage':data["vimage"], 'username':data["username"], 'useremail':data["useremail"], 'videoselector':data["videoselector"], 'queue_count':queue_count[rooms[data["room"]]]}, room=rooms[data["room"]])
